...
namespace ImGui
{
	...
	public:
		...
		bool SetPwd(const boost::filesystem::path &pwd =
		boost::filesystem::current_path());

		// get current browsing directory
		const boost::filesystem::path &GetPwd() const noexcept;
	...
	private:
		...
		void SetPwdUncatched(const boost::filesystem::path &pwd);

		bool IsExtensionMatched(const boost::filesystem::path &extension) const;
		...
		boost::filesystem::path pwd_;
		std::set<boost::filesystem::path> selectedFilenames_;
		  ...
		  inputNameBuf_(std::make_unique<std::array<char, INPUT_NAME_BUF_SIZE>>())
{
	...
	SetPwd(boost::filesystem::current_path());
...
//		if(SmallButton(u8StrToStr(sec.u8string()).c_str()))
	...
	boost::filesystem::path newPwd; bool setNewPwd = false;
...
inline bool ImGui::FileBrowser::SetPwd(const boost::filesystem::path &pwd)
{
	try
	{
		SetPwdUncatched(pwd);
	...
	SetPwdUncatched(boost::filesystem::current_path());
	return false;
}

inline const class boost::filesystem::path &ImGui::FileBrowser::GetPwd() const noexcept
{
	return pwd_;
}

inline boost::filesystem::path ImGui::FileBrowser::GetSelected() const
{
...
inline void ImGui::FileBrowser::SetPwdUncatched(const boost::filesystem::path &pwd)
{
	fileRecords_ = { FileRecord{ true, "..", "[D] ..", "" } };

	for(auto &p : boost::filesystem::directory_iterator(pwd))
	{
		FileRecord rcd;
...
inline bool ImGui::FileBrowser::IsExtensionMatched(
		const boost::filesystem::path &_extension) const
{
#ifdef _WIN32
